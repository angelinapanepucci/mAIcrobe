...
https://github.com/HoldenLab/micromorph/blob/master/src/micromorph/bacteria/phase_contrast_fitting.py
"""
...
def super_gaussian_fwhm(
    width: float, order: float or int = super_gaussian_order
) -> float:
    """
...
def super_gaussian(
    ...
    if np.isnan(value).any():
        print(
            f"nan value found for x={x}, center={center}, width={width}, order={order}, amplitude={amplitude}"
        )
    return value
